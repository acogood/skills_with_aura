...
`$ARGUMENTS` — the platform or content description (e.g., "LinkedIn", "Substack newsletter", "X threads about AI").
...
The user uploads files containing their posts. These come in messy formats — the skill handles the conversion. Accepted formats:
...
- Files in `content-workspace/sources/`
...
Upload your content data dump. This can be:
  ...
  - Or place files in content-workspace/sources/
...
Read the uploaded file(s) from wherever the user provides them.

### Step 2: Get Audience Context

Scan `content-workspace/profiles/` for `.md` files with "profile" in the filename.

**If audience profiles found:**

```
I found these audience profiles:
  1. [profile-name-1.md]
  2. [profile-name-2.md]

Use one of these for research context, or describe a different space?
  1. [profile-name-1.md]
  2. [profile-name-2.md]
  3. I'll describe the space myself

Pick a number:
```

If they pick a profile, read it and extract: the audience's industry, role, pain points, and topics they engage with. Use this to guide the research in Phase 3.

If they pick "describe the space myself", ask:
...
```bash
mkdir -p content-workspace/data
```

Save to: `content-workspace/data/[platform]_[YYYY-MM-DD].md`

Example: `content-workspace/data/substack_2026-03-12.md`
...
Converted [N] posts into a clean data file.
  ...
  Saved to: content-workspace/data/[platform]_[date].md
  Date range: [earliest] to [latest]
...
Read the clean `.md` file from `content-workspace/data/`.
...
```bash
mkdir -p content-workspace/content/content-ideas
```

**Filenames:**
- `content-workspace/content/content-ideas/lookalike-ideas-YYYY-MM-DD-[platform].md`
- `content-workspace/content/content-ideas/lookalike-ideas-YYYY-MM-DD-[platform].html`

Read the HTML styling guide bundled with this skill for the HTML version. The guide file is `html-lookalike-ideas-guide.md` in the same directory as this SKILL.md.
...
Present both files and offer next steps:
